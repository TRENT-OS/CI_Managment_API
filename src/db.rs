...
pub async fn update_hardware_status(db: &mut SqliteConnection, hardware: &str, runner: &str, status: HardwareStatus) -> Result<()> {
    let status_str = status.as_ref().to_owned();
    sqlx::query!("UPDATE Hardware SET Status = ?, ClaimedBy = ? WHERE Id = ?", 
        status_str, 
        runner, 
        hardware)
        .execute(db)
...
pub async fn get_hardware_info(db: &mut SqliteConnection, hardware: &str) -> hardware::HardwareInfo {
    let data = sqlx::query!("SELECT Id, Status, ClaimedBy FROM Hardware WHERE Id = ?", hardware)
        .fetch_one(db)
        .await.unwrap();
    
    let hw_status = HardwareStatus::from_str(&data.Status).expect("Invalid Hardware Status: Database Corruption");
